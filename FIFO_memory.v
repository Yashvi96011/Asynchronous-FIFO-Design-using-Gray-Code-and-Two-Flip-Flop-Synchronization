module fifo_mem #(
    ...
    input  wire                     wen,
    input  wire                     ren,
    input  wire [ADDR_WIDTH:0]      binary_wptr,
    input  wire [ADDR_WIDTH:0]      binary_rptr,
    ...
    always @(posedge wclk) begin
        if ((wen == 1'b1) && (full == 1'b0)) begin
            fifo_array[binary_wptr[ADDR_WIDTH-1:0]] <= data_in;
        end
    end

    // Read operation (combinational)
    assign data_out = fifo_array[binary_rptr[ADDR_WIDTH-1:0]];
